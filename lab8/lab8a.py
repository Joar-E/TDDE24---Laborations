...
def ts_overlapping_part(ts1: TimeSpan, ts2: TimeSpan) -> TimeSpan:
    ...
    # att göra detta...
    min1 = max(
        hour_number(time_hour(ts_start(ts1))) * 60 + minute_number(time_minute(ts_start(ts1))), #ts1.start.hour.number * 60 + ts1.start.minute.number,  #
    ...
    return new_time_span(Time(Hour(min1 // 60), Minute(min1 % 60)), Time(Hour(min2 // 60), Minute(min2 % 60)))#TimeSpan(Time(Hour(min1 // 60), Minute(min1 % 60)), Time(Hour(min2 // 60), Minute(min2 % 60)))
